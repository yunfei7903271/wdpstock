...
namespace wdp_Stock.Controllers
{
    ...
    public class StockController : Controller
    {
        ...
        public IEnumerable<ResearchReport> GetResearchReports()
        {
            ...
            items.Url = "http://datainterface.eastmoney.com/EM_DataCenter/js.aspx?type=SR&sty=GGSR&ps=4000&p=1&mkt=0&stat=0";//请求地址
            ...
            foreach (var item in reports.Select(c => c.SecuName).Distinct())
            {
                int count = reports.Where(x => x.SecuName == item).Count();
                reports.Where(x => x.SecuName == item).Select(q => q.Count = count).ToList();
            }
